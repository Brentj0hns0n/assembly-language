...
#             hopPtr22 = begPtr + 1;
					lw $t5, 0($fp)
					addi $t5, $t5, 4 		# t5 is temp storage (with incremented hopPtr22
					move $t2, $t5			# increment hopPtr22
...
#                      *(hopPtr222 - 1) = *hopPtr222;
					li $t5, -4
					add $t5, $t5, $t3		# t5 has *(hopPtr222 - 1(4))
					lw $t6, 0($t3)			# t6 has *hopPtr222
					sw $t6, 0($t5)
#                   ++hopPtr222;
					addi $t3, $t3, 4
#//                 }
...
#             endPtr2 = intArr + (*usedPtr);
					lw $t5, 4($sp)		#t5 has intArr
					lw $t6, 8($sp)		#t6 has usedPtr
					lw $t3, 0($t6)
					add $s2, $t5, $t3
...
#             if (hopPtr2 < endPtr2) goto begF_PAL;
					blt $t2, $s2, begF_PAL
					# EPILOG:
					lw $s2 16($sp)		#restore s2 before exiting
					lw $fp, 32($sp)				
					lw $ra, 36($sp)
					addiu $sp, $sp, 40  
					########## (27) ##########THREE LESS???
...
#                if (*hopPtr != target) goto elseI_RAO;
					lw $t0, 8($sp)		# t0 has target value
					lw $t2, 0($t1)		# t2 has *hopPtr 
					bne $t0, $t2, elseI_RAO
begI_RAO:#//     {
#                   ++remCount;
					addi $v0, $v0, 1
...
#             hopPtr1 = a1;
					lw $t1, 0($sp)
#             endPtr1 = a1 + (*used1Ptr);
					lw $t0, 0($sp)		# $t0 has a1
					lw $t3, 4($sp)		# t3 has *used1ptr
					add $t9, $t0, $t3 
...
#                *(a3 + *used3Ptr) = *hopPtr1;
					lw $t0, 8($sp)
					lw $t3, 12($sp)
					add $t3, $t3, $t0	# t3 has *(a3 + *used3Ptr)
					lw $t0, 0($t1)
					sw $t0, 0($t3)
#                ++(*used3Ptr);
					lw $t3, 8($sp)				
...
#   CopyArray(a2, used2Ptr, a1, *used1Ptr);
					...
					lw $t2, 24($sp)				# $t2 has pointer to used2ptr
					lw $a1, 0($t2)
					lw $a3, 20($sp)
					########## (3) ######DONE
